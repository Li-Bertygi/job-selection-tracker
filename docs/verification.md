...
以下は SSH / SCP ベースで検証した時点の結果です。現在の workflow は SSM Run Command ベースへ移行しています。
...
この時点の EC2 デプロイは SSH / SCP ベースだったため、検証時のみ Security Group の SSH inbound を一時的に開放しました。
その後、workflow は SSM Run Command ベースへ移行しました。

### SSM Run Command deploy workflow

`deploy-ec2.yml` は SSH private key を使わず、GitHub Actions OIDC Role から AWS Systems Manager Run Command を実行する方式へ変更しました。

現在の deploy workflow の主な step:

- Checkout
- Configure AWS credentials
  - OIDC により `AWS_ROLE_TO_ASSUME` を assume
- Login to Amazon ECR
- Set image tag
- Build and push backend image
- Build and push frontend image
- Render deployment files
  - `docker-compose.prod.yml` と `.env` を workflow runner 上で生成
- Deploy on EC2 via SSM
  - `AWS-RunShellScript` で EC2 上に deploy files を配置
  - EC2 上で ECR login
  - `docker compose pull`
  - `docker compose up -d`

SSM 方式で必要な GitHub Secret:
