...
```bash
curl.exe --% -X PATCH http://localhost:8080/stages/3 -H "Content-Type: application/json" -d "{\"status\":\"COMPLETED\"}"
```

```bash
curl.exe --% -X PATCH http://localhost:8080/stages/3 -H "Content-Type: application/json" -d "{\"status\":\"PASSED\"}"
```

#### 選考ステージステータス履歴取得

```bash
curl.exe --% http://localhost:8080/stages/3/status-histories
```
...
curl.exe --% http://localhost:8080/stages/9999/status-histories
...
- 現在は `/auth/signup` と `/auth/login` のみ実装済みであり、JWT 発行と `/auth/me` は今後の実装対象です。
...
- 現在は `signup` / `login` に必要な最小項目のみを管理する
...
- `POST /auth/login` は実装済み
- `POST /auth/refresh` と `GET /auth/me` は JWT 導入後に実装予定
